...
fun LemuroidButtonForeground(
    ...
    GlassSurface(
        ...
        fillColor = theme.foregroundFill(pressed.value).let { it.copy(alpha = it.alpha * opacity) },
        shadowColor = theme.level3Shadow.let { it.copy(alpha = it.alpha * opacity) },
...
private fun BoxWithConstraintsScope.LemuroidButtonForegroundIcon(
    ...
    Icon(
        ...
        contentDescription = "",
        tint = LocalLemuroidPadTheme.current.icons(pressedState.value).let { it.copy(alpha = it.alpha * opacity) },
    )
}
...
private fun BoxWithConstraintsScope.LemuroidButtonForegroundLabel(
    ...
    Text(
        ...
        text = label,
        color = LocalLemuroidPadTheme.current.icons(pressedState.value).let { it.copy(alpha = it.alpha * opacity) },
