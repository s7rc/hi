...
class StorageAccessFrameworkProvider(private val context: Context) : StorageProvider {
    ...
    override fun listBaseStorageFiles(): Flow<List<BaseStorageFile>> {
        ...
        return kotlinx.coroutines.flow.merge(*flows.toTypedArray())
    }
    ...
    private fun getExternalFolders(): Set<String> {
        ...
        val singleFolderKey = context.getString(R.string.pref_key_extenral_folder)
        val multiFoldersKey = context.getString(R.string.pref_key_external_folders)

        val multiFolders = prefs.getStringSet(multiFoldersKey, emptySet())?.toSet() ?: emptySet()
        val singleFolder = prefs.getString(singleFolderKey, null)

        return if (multiFolders.isNotEmpty()) {
             multiFolders
        } else if (singleFolder != null) {
            setOf(singleFolder)
        } else {
            emptySet()
        }
