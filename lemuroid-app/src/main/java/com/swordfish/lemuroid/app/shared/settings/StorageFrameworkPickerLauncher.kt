    ...
    override fun onActivityResult(
        ...
        if (requestCode == REQUEST_CODE_PICK_FOLDER && resultCode == Activity.RESULT_OK) {
            ...
            if (newValue != null) {
                ...
                contentResolver.takePersistableUriPermission(newValue, Intent.FLAG_GRANT_READ_URI_PERMISSION)

                val multiKey = getString(com.swordfish.lemuroid.lib.R.string.pref_key_external_folders)
                val currentSet = sharedPreferences.getStringSet(multiKey, emptySet()) ?: emptySet()
                val newSet = currentSet + newValue.toString()

                sharedPreferences.edit().apply {
                    this.putStringSet(multiKey, newSet)
                    // Also update single key for backward compatibility or as "primary"
                    this.putString(preferenceKey, newValue.toString())
                    this.apply()
                }
                android.util.Log.d("LemuroidStorage", "Saved ${newSet.size} folders: $newSet")
                android.widget.Toast.makeText(this, "Added Folder. Total: ${newSet.size}", android.widget.Toast.LENGTH_LONG).show()
            }
