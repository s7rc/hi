...
fun SettingsScreen(
    ...
    LemuroidSettingsPage(modifier = modifier) {
        RomsSettings(
            ...
            onClearFolders = { viewModel.clearLocalStorageFolders() },
...
private fun RomsSettings(
    ...
    val currentDirectoryName =
        remember(state.directories) {
            if (directories.isEmpty()) {
                emptyDirectory
            } else {
                directories.mapNotNull { uriString ->
                    runCatching {
    ...
    LemuroidCardSettingsGroup(title = { Text(text = stringResource(id = R.string.roms)) }) {
        LemuroidSettingsMenuLink(
            title = { Text(text = "Directories (Tap to Add)") },
        ...
        if (directories.isNotEmpty()) {
             LemuroidSettingsMenuLink(
                title = { Text(text = "Clear All Directories") },
